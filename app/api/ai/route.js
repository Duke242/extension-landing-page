...
export async function POST(req) {
  try {
    ...
    const prompt = ChatPromptTemplate.fromTemplate(
      `You are an excellent writer and also an excellent grader of writing. Grade the following writing on a scale of 1-100. Also, rewrite the passage in what you think will be a score of 90-100. In the suggestion only include your rewritten suggestion of the passage and nothing else. Grade the writing on clarity, conciseness, coherence and simple vocabularly. Format your response as a JSON object with "rating", "feedback", and "suggestion" fields. Only use words and no markdown or anything else like that. Passage: {input}`
    )
